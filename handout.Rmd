...
- Include information about data sharing in informed consent (for examples, see [A practical guide for transparency in psychological science](http://psych-transparency-guide.uni-koeln.de//informed-consent.html))
...
- Processed data
    ...
    - Ideally ["Tidy Data"](https://www.jstatsoft.org/index.php/jss/article/view/v059i10/v59i10.pdf)
...
- Participant privacy
    ...
    - Only share anonymized data publicly, e.g.
        - Delete directly identifying information (e.g., full name, address, images of the face, handwriting, genetic information)
        - note that combinations of simple indirect identifiers (e.g., ZIP code + gender + DOB, see [Sweeney, 2000](https://dataprivacylab.org/projects/identifiability/paper1.pdf)) can also be problematic
        - Separate demographic data used for sample description and study data
        ...
        - If necessary, transform data, e.g.
            ...
            - Add noise
            - Aggregation
        - for a more sophisticated approach you can also use anonymization tools, such as [Amnesia](https://amnesia.openaire.eu/index.html)
...
- Automate codebook generation
    ...
    <!-- TODO: add screenshots for SPSS codebook functions -->
...
- Describe the default practices of a group in data analysis (e.g., [SOP for Green lab at Columbia](http://alexandercoppock.com/Green-Lab-SOP/Green_Lab_SOP.pdf))
...
- Have a basic data management plan, ideally at the start of your project/study ([Jones, 2011](http://www.dcc.ac.uk/resources/how-guides), https://www.icpsr.umich.edu/icpsrweb/content/datamanagement/dmp/framework.html, http://www.dcc.ac.uk/sites/default/files/documents/resource/DMP/DMP-checklist-flyer.pdf) <!-- TODO: Can we give a simple example here? Provide additional guidance? -->
    - Examples: https://dmponline.dcc.ac.uk/public_plans
    - Project structure
        - not necessary to create completely new DMP for every study/project (esp. if they are similar): create & use basic template
    - ZPID (Leibniz Psychology) data management tool [DataWiz](https://datawiz.leibniz-psychology.org/DataWiz/?datawiz_locale=en)
...
```{r sharing option ratings, echo = F}

Sharing_option <- c("On request", "Personal website", "Journal website", "Institutional repository", "Public repository")
Findability <- c("--", "-", "+", "+", "++")
Sustainability <- c("--", "-", "0", "++", "++")
Access_control <- c("++", "0", "0", "+", "+")
sharing_options <- data.frame(Sharing_option, Findability, Sustainability, Access_control, stringsAsFactors = F)
...
<!-- maybe make a prety table instead of just printing the dataframe? -->
...
- Data & Metadata
    - Born-open data
        - **Demonstration**: Set up Dropbox to collect born-open data <!-- TODO: Johannes, ist das eher when oder how? Johannes: eher when, wuerde ich sagen -->
- Analysis scripts
